...
Optimization), a C++ solver for nonlinearly constrained optimization, via Uno's
C API. You describe a nonlinear program with R callbacks --- objective,
gradient, constraints, Jacobian and Lagrangian Hessian --- and Uno solves it.
This is the R analog of the `unopy` Python binding, intended as the nonlinear
...
A C++17 compiler and CMake (>= 3.16) are required: the package builds Uno and
HiGHS from source. MUMPS comes from the `rmumps` dependency, so there is no
separate MUMPS installation.
